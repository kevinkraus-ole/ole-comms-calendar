...
CREATE POLICY "Public can read" ON communications
  FOR SELECT USING (true);

CREATE POLICY "Authenticated can insert" ON communications
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated can update" ON communications
  FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY "Authenticated can delete" ON communications
  FOR DELETE USING (auth.role() = 'authenticated');

-- Seed data
INSERT INTO communications (title, description, channel, segment, scheduled_at, status, created_by) VALUES
